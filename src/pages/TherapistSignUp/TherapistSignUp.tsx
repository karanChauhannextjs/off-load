...
const TherapistSignUp: React.FC = () => {
  ...
  const exerciseCompleteData = JSON.parse(
    localStorage.getItem('exerciseCompleteData') ?? '{}',
  );
  const checkinCompleteData = JSON.parse(
    localStorage.getItem('checkinCompleteData') ?? '{}',
  );
  ...
  const handleFormSubmit = async (data: ITherapistSignupForm) => {
    try {
      ...
      if (
        ...
        error.message === 'Go to verification'
      ) {
        navigate(`${USER_PUBLIC_BASE_URL}/${RoutesEnum.VERIFICATION}`, {
          state: {
            email: data.email,
        ...
        <div className={styles.rightWrapper}>
          <div className={styles.titleBlock}>
            <span className={styles.title}>
              Create your free therapist account
            </span>
